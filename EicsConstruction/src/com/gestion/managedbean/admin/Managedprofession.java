...
@Component
public class Managedprofession implements Serializable {
	/**
	 * 
	 */
	...
	private static Logger logger = Logger.getLogger(Managedprofession.class);
...
// geters et seters
	...
	public List<Profession> getListeprofession() {
		...
		listeprofession = new ArrayList<Profession>();
		List<Object> listObject = getObjectService().getObjects("Profession");
		for (Iterator it = listObject.iterator(); it.hasNext();) {
			Profession profession = (Profession) it.next();
			try {
				listeprofession.add(profession);
			} catch (Exception e) {
			}
