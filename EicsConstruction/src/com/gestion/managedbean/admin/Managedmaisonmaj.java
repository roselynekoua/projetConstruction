...
@Component
public class Managedmaisonmaj implements Serializable {
	...
	private static Logger logger = Logger.getLogger(Managedmaisonmaj.class);
	private Maison maison = new Maison();
	
	
	private List listemaison = new ArrayList<>();
	...
	public void enregistrer(){
		try {
			...
			maison.setCodeMais(getIdGenerateur().getIdmaison());
...
// geters et seters
	...
	public List getListemaison() {
		
		
		
		listemaison = new ArrayList<Maison>();
		List<Object> listObject = getObjectService().getObjects("Maison");
		for (Iterator it = listObject.iterator(); it.hasNext();) {
			Maison maison = (Maison) it.next();
			try {
	...
	public void setListemaison(List listemaison) {
