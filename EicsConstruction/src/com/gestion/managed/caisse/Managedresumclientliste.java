...
@Component
public class Managedresumclientliste implements Serializable {
	...
	private List<Facture> lisFacture = new ArrayList<>();
	...
	public void chargerliste() {
		getObjectService().getojects(client);
...
//				contrat.getQteTerSup()));
	...
	public void onRowSelectclt(SelectEvent event) {
		try {
			...
			if ((getRequeteUtilisateur().recuplistcontrat(client.getNumClt()).size()) != 0) {
				setListecontrat(getRequeteUtilisateur().recuplistcontrat(
						client.getNumClt()));
			} else {
				FacesContext.getCurrentInstance().addMessage(
						null,
						new FacesMessage(FacesMessage.SEVERITY_INFO,
								"Avertissement",
								"il n'y a pas de contrat pour ce client"));

			}

			// setListecontrat((List<Contrat>) getClient().getContrats());
			setLisFacture(getRequeteUtilisateur().recuplistfacture(
					client.getNumClt()));
			if (lisFacture.size() == 0) {
			...
			FacesContext.getCurrentInstance().addMessage(
					null,
					new FacesMessage(FacesMessage.SEVERITY_INFO,
							"Avertissement", "Une erreur s'est produite"));
		}
	}
	...
	public List<Client> getListeclient() {
		 ...
		 * 
		 * 
		 * if (listeclient.isEmpty()) { listeclient =
		 * getObjectService().getObjects("Client"); } listeclient= new
		 * ArrayList<Client>(); List<Object> listObject =
		 * getObjectService().getObjects("Client"); for (Iterator it =
		 * listObject.iterator(); it.hasNext();) { Client clt = (Client)
		 * it.next(); try { listeclient.add(clt); } catch (Exception e) { } }
		 */
		return listeclient;
	}
	...
	public List<Facture> getLisFacture() {
		return lisFacture;
	}

	public void setLisFacture(List<Facture> lisFacture) {
		this.lisFacture = lisFacture;
	}
